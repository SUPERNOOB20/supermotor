...
struct SDL_Application{
		...
		while (SDL_PollEvent(&event)){
			if (event.type == SDL_EVENT_QUIT){
                ...
                if (event.button.button == 80){      
                    gallery--;      // Press the left key to move to the previous image.
                } else if (event.button.button == 79){          
                    gallery++;      // Press the right key to move to the next image.
                }
            }
        }
	}
	...
	void Update(){
        gallery = gallery % NUMBER_OF_IMAGES;            // Loops the gallery around to keep the indices small.
        if (gallery < 0){
            gallery = NUMBER_OF_IMAGES + gallery;       //  Usually this silly extra step wouldn't be needed, but modulo in cpp is... weird? For negative numbers ":3
        }
	}
	...
	void Render(){
        ...
        for (int i = 0; i < 4; i++){
            ...
            texture_srcrect.w = (float) WINDOW_WIDTH  * 10.0f;
            texture_srcrect.h = (float) WINDOW_HEIGHT * 10.0f;

            // SDL_RenderTexture(mRenderer, your_textures_here[i], &texture_srcrect, nullptr);
            SDL_RenderTexture(mRenderer, your_textures_here[gallery], &texture_srcrect, nullptr);
	...
	void Tick(){
        ...
        if (my_log == true){
            SDL_Log("current gallery item ID: %d", gallery);
            my_log = false;
        }
	}
